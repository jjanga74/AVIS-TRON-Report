# AVIS-TRON PDF Reporting System 사용 설명서 (User & Developer Manual)
...
### 3.2. 템플릿 선택, 방향 및 데이터 커스텀
*   **1. 리포트 템플릿 및 방향 설정:** 
    *   **리포트 템플릿 선택:** '비즈니스 송장' 또는 '월간 매출 보고서' 탭을 선택합니다.
    *   **인쇄 방향 (Orientation):** '세로 인쇄' 또는 '가로 인쇄' 버튼을 클릭하여 출력 방향을 지정합니다. 가로 인쇄 선택 시 서버에서 자동으로 가로 맞춤 레이아웃으로 렌더링을 조정합니다.
*   **2. 데이터 상세 설정:** 
...
*   **A4 규격 및 인쇄 방향 대응:**
    ```css
    @page {
      size: A4 portrait; /* 기본 설정 (가로 인쇄 요청 시 서버에서 size: A4 landscape로 자동 변환됨) */
      margin: 25mm 15mm 20mm 15mm; /* 헤더/푸터 영역 공간 확보 */
    }
    ```
    가로 인쇄가 선택되면 서버(`server.js`) 측에서 `@page` 규칙의 `portrait`를 `landscape`로 동적으로 치환하고 `<body>` 태그에 `landscape` 클래스를 주입합니다 (`<body class="landscape">`). 이를 활용해 미디어 쿼리나 상위 클래스 지정을 통해 가로 모드에 특화된 레이아웃을 작성할 수 있습니다.
    
    *예시 (가로 모드 시 차트 크기 확장):*
    ```css
    body.landscape .chart-wrapper {
      max-width: 750px;
    }
    ```
